...
$success_message->message   = 'Thanks! your booking has been forwarded';
...
if (!empty($fields)) {
    ...
    if (empty($returnVal->messages)) {                         // Enable encryption, 'ssl' also accepted
        $datepickerin = stripslashes(safe($fields['datepickerin']));
        $datepickerout = stripslashes(safe($fields['datepickerout']));
        $type = stripslashes(safe($fields['type']));
        // The sender of the form/mail
        $mail->From = 'New guest';
        $mail->FromName = 'Guest';
        $mail->Subject = '[' . $company_name . ']';
        $content = "A new guest sent you a message. He would like to check in on:<br>";
        $content .= "<br><strong>" . $datepickerin . "</strong> and check out on <strong>" . $datepickerout . "</strong><br>
        for a <strong>". $type ."</strong><br><br><br>";
        $mail->Body = $content;
