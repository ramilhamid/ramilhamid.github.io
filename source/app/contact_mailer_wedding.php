...
if (!empty($fields)) {
    ...
    if (empty($returnVal->messages)) {                         // Enable encryption, 'ssl' also accepted
        ...
        $firstname = stripslashes(safe($fields['firstname']));
        $lastname = stripslashes(safe($fields['lastname']));
        $guests = stripslashes(safe($fields['guests']));
        $events = stripslashes(safe($fields['events']));
        // The sender of the form/mail
        $mail->From = $email;
        $mail->FromName = $lastname;
        $mail->Subject = '[' . $company_name . ']';
        $content = $lastname . "" . $firstname . " sent you a message. He would like to check in for:<br>";
        $content .= "<br><strong>" . $guests . "</strong> guests at the <strong>" . $events . "</strong> party.<br>";
        $mail->Body = $content;
